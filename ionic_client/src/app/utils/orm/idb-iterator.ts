 ...
 *
 */
export class IDBIterator implements AsyncIterableIterator<unknown> {
  private cursorRequest: IDBRequest;

  private cursor: IDBCursorWithValue;

  private curResolver: (result: IteratorResult<unknown>) => void;
  ...
  public next(): Promise<IteratorResult<unknown>> {
    ...
    return new Promise<IteratorResult<unknown>>(resolve => {
      ...
      if (this.cursorRequest) {
        // 2nd request or later -> continue (Still, the success
        // listener below will be called when the cursor has been moved forward):
        ...
        this.cursorRequest.onerror = (e): void => {
          throw e;
        };

        this.cursorRequest.onsuccess = (): void => {
          this.cursor = this.cursorRequest.result;
          if (this.cursor) {
